...
public class TileTranslocatorRenderer extends TileEntitySpecialRenderer {
    ...
    public static Vector3[] sidePos = new Vector3[] { new Vector3(0.5, 0, 0.5), new Vector3(0.5, 1, 0.5),
            new Vector3(0.5, 0.5, 0), new Vector3(0.5, 0.5, 1), new Vector3(0, 0.5, 0.5), new Vector3(1, 0.5, 0.5) };
    public static Vector3[] sideVec = new Vector3[] { new Vector3(0, -1, 0), new Vector3(0, 1, 0),
    ...
    static {
        Map<String, CCModel> models = CCModel
                .parseObjModels(new ResourceLocation("translocator", "models/model.obj"), new SwapYZ());
    ...
    public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float f) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (a != null) renderAttachment(
                    i,
                    ttrans.getBlockMetadata(),
    ...
    private void renderLink(int src, int dst, double time, int x, int y, int z) {
        ...
        for (int n = 0; n < 20; n++) {
            ...
            if (!MathHelper.between(0, dn, 1) || spriteX < 0) continue;
            ...
            RenderParticle.render(
                    ...
                    pos.y,
                    pos.z,
                    gradient.getColour((dn - 0.5) * 1.2 + 0.5).multiplyC(b),
                    s * 0.12,
                    u1,
                    v1,
                    u2,
    ...
    public static Vector3 getPerp(int src, int dst) {
        if ((src ^ 1) == dst) return sideVec[(src + 2) % 6].copy();

        for (int i = 0; i < 3; i++) if (i != src / 2 && i != dst / 2) return sideVec[i * 2].copy();

        return null;
    }

    private static Vector3 getPathNormal(int src, int dst, double d) {
        if ((src ^ 1) == dst) return sideVec[(src + 4) % 6].copy();
    ...
    public static void renderAttachment(int i, int type, double insertpos, int field, double x, double y, double z) {
        ...
        Matrix4 matrix = new Matrix4().translate(new Vector3(x + 0.5, y + 0.5, z + 0.5)).apply(sideRotations[i])
                .translate(new Vector3(0, -0.5, 0)).scale(new Vector3(1, insertpos * 2 / 3 + 1 / 3D, 1));
