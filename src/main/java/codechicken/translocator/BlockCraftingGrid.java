...
public class BlockCraftingGrid extends Block {
    ...
    public List<IndexedCuboid6> getParts(World world, int x, int y, int z) {
        ...
        parts.add(new IndexedCuboid6(0, new Cuboid6(0, 0, 0, 1, 0.005, 1).add(new Vector3(x, y, z))));

        TileCraftingGrid tcraft = (TileCraftingGrid) world.getTileEntity(x, y, z);

        for (int i = 0; i < 9; i++) {
            Cuboid6 box = new Cuboid6(1 / 16D, 0, 1 / 16D, 5 / 16D, 0.01, 5 / 16D).apply(
                    new Translation((i % 3) * 5 / 16D, 0, (i / 3) * 5 / 16D)
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (hit != null) {
            if (hit.subHit > 0) tcraft.activate(hit.subHit - 1, player);
            return true;
        }
        return false;
    }
    ...
    public boolean placeBlock(World world, EntityPlayer player, int x, int y, int z, int side) {
        if (disableCraftingGridKey) return false;

        Block block = world.getBlock(x, y, z);
        if (side != 1 && block != Blocks.snow_layer) return false;

        if (block != Blocks.vine && block != Blocks.tallgrass
        ...
        if (!world.canPlaceEntityOnSide(this, x, y, z, false, 1, null, null)) return false;
    ...
    public void onBlockPreDestroy(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && c != null && c.equals(new BlockCoord(x, y, z))) dropBlockAsItem(world, x, y, z, meta, 0);
