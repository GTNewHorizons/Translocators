...
public class TileItemTranslocator extends TileTranslocator {
    ...
    public class ItemAttachment extends Attachment {
        ...
        public Collection<ItemStack> getDrops() {
            ...
            if (regulate) stuff.add(new ItemStack(Translocator.itemDiamondNugget));
            if (signal) stuff.add(new ItemStack(Items.iron_ingot));
            return stuff;
        }
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        if (tag.hasKey("items")) for (Attachment a : attachments) if (a != null)
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote) {
            for (Iterator<MovingItem> iterator = movingItems.iterator(); iterator.hasNext();)
                if (iterator.next().update()) iterator.remove();
        } else {
            // move those items
            ...
            for (int i = 0; i < 6; i++) {
                ...
                for (int slot : access.slots) {
                    ...
                    if (stack == null || !access.canExtractItem(slot, stack) || stack.stackSize == 0)// stack size 0
                                                                                                     // hack
                        continue;

                    int quantity = ia.fast ? stack.stackSize : 1;
                    if (quantity <= largestQuantity) continue;
    ...
    private boolean hasEmptySpace(InventoryRange inv) {
        for (int slot : inv.slots) {
            ...
            if (inv.canInsertItem(slot, new ItemStack(Items.diamond)) && (stack == null || stack.isStackable()
    ...
    private int filterCount(ItemAttachment ia, ItemStack stack) {
        ...
        for (ItemStack filter : ia.filters) if (filter != null) {
            filterSet = true;
            if (matches(stack, filter)) match += filter.stackSize;
        }
    ...
    private void spreadOutput(ItemStack move, int src, boolean rspass, InventoryRange[] attached) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (ia != null && !ia.a_eject && ia.redstone == rspass) {
                ...
                if (outputQuantities[i] > 0) outputCount++;
            }
        }
    ...
    private int extractAmount(ItemStack stack, ItemAttachment ia, InventoryRange range) {
        int filter = filterCount(ia, stack);
        if (filter == 0) return ia.regulate ? stack.getMaxStackSize() : 0;
    ...
    public void handleDescriptionPacket(PacketCustom packet) {
        if (packet.getType() == 2) movingItems.add(new MovingItem(packet));
        else super.handleDescriptionPacket(packet);
    }

    @Override
    public int strongPowerLevel(int side) {
        ItemAttachment ia = (ItemAttachment) attachments[side ^ 1];
        if (ia != null && ia.a_powering) return 15;
        return 0;
    }
}
