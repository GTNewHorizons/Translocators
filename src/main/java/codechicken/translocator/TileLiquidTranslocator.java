...
public class TileLiquidTranslocator extends TileTranslocator implements IFluidHandler {
    ...
    public class MovingLiquid {
        ...
        public void addLiquid(int moving) {
            if (liquid.amount == 0) throw new IllegalArgumentException("Something went wrong!");

            liquid.amount += moving;
            if (attachments[src] != null) fast = attachments[src].fast;
            capLiquid();
        }
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote) {
            ...
            if (!transfers.isEmpty()) sendTransferPacket(transfers);
        }
    }

    private void spreadOutput(FluidStack move, int src, int[] outputs, TankAccess[] attached,
            ArrayList<LiquidTransfer> transfers) {
    ...
    public int[] sortOutputs(boolean b) {
        ...
        for (int i = 0; i < 6; i++) {
            Attachment a = attachments[i];
            if (a != null && !a.a_eject && a.redstone == b) str[k++] = i;
        }
    ...
    public void handleDescriptionPacket(PacketCustom packet) {
        if (packet.getType() == 2) {
            ...
            for (LiquidTransfer t : transfers) {
                ...
                if (!found && attachments[src] != null) movingLiquids.add(new MovingLiquid(src, dst, t.liquid));
            }
        } else {
    ...
    public FluidTankInfo[] getTankInfo(ForgeDirection from) {
        if (from == ForgeDirection.UNKNOWN) {
            ...
            for (Attachment a : attachments) if (a != null) list.add(new FluidTankInfo(null, 0));
