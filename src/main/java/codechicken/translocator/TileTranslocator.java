...
public abstract class TileTranslocator extends TileEntity implements ICustomPacketTile {
    ...
    public class Attachment {
        ...
        public void read(PacketCustom packet, boolean described) {
            ...
            if (!described) a_insertpos = b_insertpos = approachInsertPos();
        }
        ...
        public boolean activate(EntityPlayer player, int subPart) {
            ...
            if (held == null && player.isSneaking()) {
                ...
                if (subPart == 1) invert_redstone = !invert_redstone;
                else openGui(player);
            } else if (held.getItem() == Items.redstone && !redstone) {
                redstone = true;
                if (!player.capabilities.isCreativeMode) held.stackSize--;

                if ((gettingPowered() != invert_redstone) != a_eject) invert_redstone = !invert_redstone;
                markUpdate();
        ...
        public void stripModifiers() {
            if (redstone) {
                ...
                if (invert_redstone != a_eject) invert_redstone = !invert_redstone;
            }
            if (fast) {
                fast = false;
        ...
        public Collection<ItemStack> getDrops() {
            ...
            if (redstone) items.add(new ItemStack(Items.redstone));
            if (fast) items.add(new ItemStack(Items.glowstone_dust));
            return items;
        }

        public int getIconIndex() {
            int i = 0;
            if (redstone) i |= gettingPowered() ? 2 : 1;
            if (fast) i |= 4;
            return i;
        }
    }
    ...
    public void updateEntity() {
        for (Attachment a : attachments) if (a != null) a.update(worldObj.isRemote);
    }
    ...
    public Packet getDescriptionPacket() {
        ...
        int attachmask = 0;
        for (int i = 0; i < 6; i++) if (attachments[i] != null) attachmask |= 1 << i;

        packet.writeByte(attachmask);
        for (Attachment a : attachments) if (a != null) a.write(packet);
        ...
        if (packet.getType() == 1) {
            ...
            for (int i = 0; i < 6; i++) {
                if ((attachmask & 1 << i) != 0) {
                    ...
                    if (!described) createAttachment(i);
                    attachments[i].read(packet, described);
                } else attachments[i] = null;
            }
    ...
    public void writeToNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < 6; i++)
            if (attachments[i] != null) tag.setTag("atmt" + i, attachments[i].write(new NBTTagCompound()));
    }
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < 6; i++) if (tag.hasKey("atmt" + i)) {
            createAttachment(i);
    ...
    public boolean harvestPart(int i, boolean drop) {
        Attachment a = attachments[i];
        if (!worldObj.isRemote && drop) for (ItemStack stack : a.getDrops()) dropItem(stack);

        attachments[i] = null;

        worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
        for (Attachment a1 : attachments) if (a1 != null) return false;
    ...
    public boolean connectRedstone() {
        for (Attachment a : attachments) if (a != null && a.redstone) return true;
        return false;
    }
